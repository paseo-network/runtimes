...
fn metadata_key<T: Config>(seed: u32) -> MetadataKeyOf<T> {
	...
	bytes.try_into().expect("key uses the configured maximum length")
}

fn metadata_value<T: Config>(byte: u8, len: u32) -> MetadataValueOf<T> {
	vec![byte; len as usize].try_into().expect("value length is benchmark-bounded")
}
...
mod benchmarks {
	...
	fn define_item(m: Linear<0, 100>) -> Result<(), BenchmarkError> {
		...
		#[extrinsic_call]
		_(RawOrigin::Signed(caller), collection, Transferability::Transferable, metadata);
	...
	fn transfer_by_holder() -> Result<(), BenchmarkError> {
		...
		let (collection, item) = create_definition::<T>(&owner)?;
		let instance =
			Pallet::<T>::do_mint(owner.clone(), collection, item, from.clone(), Vec::new())?;
		...
		assert_eq!(nft.state_nonce, 1);
		assert_eq!(Instances::<T>::get(instance), Some(to));
		assert!(!NftsByOwner::<T>::contains_key(&from));
		assert!(!Locked::<T>::contains_key(&from));
		Ok(())
	}
