...
fn fund_native(who: u64, amount: u64) {
...
fn member_key_already_used_fail() {
	new_test_ext().execute_with(|| {
		let signer1 = 1;
		let signer2 = 2;
		fund_native(signer1, 1000);
		fund_native(signer2, 1000);
...
fn success_accounting() {
	new_test_ext().execute_with(|| {
		...
		fund_native(signer, initial_bal);

		let fee_native = Coinage::paid_unload_token_fee_in_native();

		let fee_dest = get_u64::<<Test as Config>::FeeDestination>();
		let dest_initial = Balances::balance(&fee_dest);
		...
		assert_eq!(Balances::balance(&signer), initial_bal - fee_native);
		assert_eq!(Balances::balance(&fee_dest), dest_initial + fee_native);
		System::assert_has_event(
			crate::Event::<Test>::PaidUnloadTokenRegisteredWithNative {
				who: signer,
				fee: fee_native,
			}
			.into(),
...
fn success_token_is_usable_in_unload_call() {
	new_test_ext().execute_with(|| {
		...
		// 3. Setup Recycler (for dummy unload call validation)
		...
		// Create alias proof first (recycler proof signs proven_msg).
		...
		let alias_proofs: BoundedVec<_, <Test as Config>::MaxConsolidation> =
			BoundedVec::try_from(vec![r_proof]).unwrap();

		// Compute intent message: blake2_256(alias_proofs.encode() ++ inherited.encode())
		let intent_msg =
...
fn success_with_previous_revision() {
	new_test_ext().execute_with(|| {
		...
		// 2. Build Ring (revision becomes 0)
		...
		// 3. Add another member and build again (revision becomes 1, previous_root is set)
		let signer2 = 2;
		fund_native(signer2, 1000);
