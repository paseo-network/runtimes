...
//! The mock runtime the game and the credits earned in it are both tested on.
//!
//! It is a source file rather than a crate: a pallet's unit tests and the runtime they run on have
...
use super::{
	indiv_pallet_game, Airdrop, AllPalletsWithSystem, Assets, AssetsHolder, Balances,
	ChunksManager, Game, Members, MockNftClaimCredits, OriginCaller, PalletInfo, People,
	RuntimeCall, RuntimeEvent, RuntimeFreezeReason, RuntimeGenesisConfig, RuntimeHoldReason,
	RuntimeOrigin, RuntimeTask, System, Test,
};
...
use sp_statement_store::{runtime_api::StatementSource, Statement, StatementAllowance, Topic};
use std::{
	cell::RefCell,
	sync::{Arc, Mutex},
	time::Duration,
...
impl WeightInfo for MockWeightInfo {
	...
	fn shuffle_step_compute_weights(_i: u32) -> Weight {
		Weight::from_parts(10, 10)
	}
	...
	fn player_process_step1_inner_loop(_r: u32) -> Weight {
		Weight::from_parts(10, 10)
	}
	...
	// Distinct base values and non-zero per-`n` slopes so the per-branch, count-based
	// pre-dispatch charge is observable in tests.
	...
	fn sign_up_with_account_lite_invite(n: u32) -> Weight {
		Weight::from_parts(35, 35)
			.saturating_add(Weight::from_parts(10, 10).saturating_mul(n as u64))
	}

	fn report(e: u32, n: u32) -> Weight {
		// `n` (early enactments) is the refundable axis; `e` (co-player entries) is known
		// exactly pre-dispatch and never refunded. Encode them on separate dimensions so
		// tests can observe the refund on `n` alone.
		Weight::from_parts(n as u64, e as u64)
	}

	// Distinct values so the per-branch refund is observable; `person` is the
	// worst case (the pre-dispatch `.max` charge), `account` the cheaper path.
	...
	// Distinct non-zero per-`n` values so the extension's count-based charge is observable in
	// tests.
	...
	fn on_game_cancelled(_n: u32) -> Weight {
		Weight::zero()
	}
}
...
thread_local! {
	/// The transactions the offchain worker submitted. See [`pool_transactions`].
	static TRANSACTION_POOL: RefCell<alloc::sync::Arc<parking_lot::RwLock<PoolState>>> =
		RefCell::new(alloc::sync::Arc::new(parking_lot::RwLock::new(PoolState {
	...
	/// The events of the blocks already advanced past, which `frame_system` drops when it
	/// initialises the next one. See [`recorded_events`].
	static PAST_EVENTS: RefCell<Vec<RuntimeEvent>> = const { RefCell::new(Vec::new()) };
	/// How many of `System::events()` [`PAST_EVENTS`] already holds, so that recording twice
	/// over one block's events records each of them once.
	static PAST_EVENTS_CURSOR: RefCell<usize> = const { RefCell::new(0) };
}

/// The raw transactions the offchain worker has submitted, without draining them.
pub fn pool_transactions() -> Vec<Vec<u8>> {
	TRANSACTION_POOL.with_borrow(|pool| pool.read().transactions.clone())
}

/// The calls of the transactions the offchain worker has submitted.
...
/// Empties the transaction pool, so that a later assertion only sees fresh submissions.
pub fn clear_pool() {
	TRANSACTION_POOL.with_borrow_mut(|pool| pool.write().transactions.clear());
}

/// Every event the test has seen, from the first block to the current one.
///
/// A block's events are the durable record of what the credits of that block were: the pallet
/// keeps their leaves only until the tree is committed, and their slots only until the game
/// ends. Assertions about individual credits therefore outlive the state they came from only
/// here, the same way a wallet's do.
pub fn recorded_events() -> Vec<RuntimeEvent> {
	record_events();
	PAST_EVENTS.with(|past| past.borrow().clone())
}

/// Take the current block's events that [`PAST_EVENTS`] does not hold yet, before
/// `frame_system` drops them with the block.
fn record_events() {
	let events = System::events();
	PAST_EVENTS_CURSOR.with(|cursor| {
		let mut cursor = cursor.borrow_mut();
		// A shorter list than what was recorded means `frame_system` dropped the previous
		// block's events, so the ones now there are all new.
		if events.len() < *cursor {
			*cursor = 0;
		}
		PAST_EVENTS.with(|past| {
			past.borrow_mut()
				.extend(events[*cursor..].iter().map(|record| record.event.clone()))
		});
		*cursor = events.len();
	});
}

/// Forget the events of a previous test running on this thread.
fn reset_recorded_events() {
	PAST_EVENTS.with(|past| past.borrow_mut().clear());
	PAST_EVENTS_CURSOR.with(|cursor| *cursor.borrow_mut() = 0);
}
...
/// `n` airdrops built from [`test_airdrop_prize`], drawn one day apart, for test schedules.
pub fn test_airdrops(
	n: u32,
) -> frame_support::BoundedVec<GameAirdrop<u32, u128>, ConstU32<{ MAX_GAME_AIRDROPS as u32 }>> {
	(0..n)
		.map(|i| GameAirdrop {
			draw_offset: i * 86_400,
			claim_window: TEST_AIRDROP_CLAIM_WINDOW as u32,
...
#[cfg(feature = "runtime-benchmarks")]
impl
	indiv_pallet_game::benchmarking::BenchmarkHelper<
		AccountAuthority,
		TestSignature,
		u64,
		AccountId32,
...
/// [`indiv_pallet_people_lite::Config::AccountContexts`]. The score context is needed for a lite
/// person to invite an account to play with `sign_up_with_account_lite_invite`.
...
impl indiv_pallet_people_lite::Config for Test {
	...
	type LiteRingExponent = LiteRingExp;
	type LiteOnboardingSize = ConstU32<7>;
...
/// The statements a test's externalities hold, standing in for the node's statement store.
///
/// One per test, not one per process: `cargo test` runs tests as threads of one process, and a
/// store shared between them lets one test's statements show up in another's assertions.
/// [`mock_statement_store`] hands out the current thread's, which [`new_test_ext`] replaces.
pub struct MockStatementStore(Mutex<Vec<(sp_statement_store::Hash, Statement)>>);
...
impl MockStatementStore {
	...
	/// The statements held, in the order they were added.
	pub fn statements(&self) -> Vec<Statement> {
		self.0.lock().unwrap().iter().map(|(_, statement)| statement.clone()).collect()
	}
}

thread_local! {
	/// See [`MockStatementStore`].
	static MOCK_STATEMENT_STORE: RefCell<Arc<MockStatementStore>> =
		RefCell::new(Arc::new(MockStatementStore(Mutex::new(Default::default()))));
}

/// The statement store the current test's externalities are wired to.
pub fn mock_statement_store() -> Arc<MockStatementStore> {
	MOCK_STATEMENT_STORE.with_borrow(Arc::clone)
}
...
pub fn new_test_ext() -> sp_io::TestExternalities {
	...
	// A thread runs one test after another under `cargo test`, so the store is replaced rather
	// than cleared: a test starts with nothing another one left behind.
	MOCK_STATEMENT_STORE
		.with_borrow_mut(|store| *store = Arc::new(MockStatementStore(Mutex::new(Vec::new()))));
	...
	// The offchain worker submits the credit tree deliveries as authorized transactions, so it
	// needs somewhere to submit them to.
...
// A helper that runs a full game scenario: "schedule" -> "sign_up" -> "report" -> "process".
#[allow(clippy::too_many_arguments)]
	...
	FR: FnOnce(),
{
	...
	MOCK_UNIX_TIME.with(|t| *t.borrow_mut() = Duration::from_secs((registration_ends + 1) as u64));
...
/// Create a runtime origin as a lite alias in the given context.
pub fn runtime_origin_for_lite_alias_in_context(
	alias: &Alias,
	context: &indiv_support::traits::Context,
) -> RuntimeOrigin {
	indiv_pallet_people_lite::Origin::LiteAlias(RevisedContextualAlias {
		revision: 0,
		ring: 0,
		ca: ContextualAlias { context: *context, alias: *alias },
	})
	.into()
}
...
impl indiv_pallet_game::Config for Test {
	type NftClaimCredits = MockNftClaimCredits;
