...
use crate::{
	tests::{
		...
		asset_hub_kusama_location, asset_hub_paseo_global_location,
		create_foreign_on_ah_paseo, snowbridge_common::*,
	},
	*,
};
...
fn send_token_to_kusama_v2() {
	...
	// set XCM versions
	BridgeHubPaseo::force_xcm_version(asset_hub_paseo_location(), XCM_VERSION);
	...
	// Register the token on AH Paseo and Kusama
	...
	AssetHubKusama::set_foreign_asset_reserves(
		...
		vec![(asset_hub_paseo_global_location(), false).into()],
	);

	// To satisfy ED
	let sov_ahw_on_ahr = AssetHubPaseo::sovereign_account_of_parachain_on_other_global_consensus(
		Kusama,
		AssetHubKusama::para_id(),
	);
	AssetHubPaseo::fund_accounts(vec![(sov_ahw_on_ahr.clone(), INITIAL_FUND)]);

	register_foreign_asset(token_location.clone(), ethereum_sovereign, false);
	set_up_eth_and_dot_pool_on_paseo_asset_hub();
...
fn send_ksm_from_ethereum_to_kusama() {
	...
	let bridged_roc_at_asset_hub_paseo = bridged_ksm_at_ah_paseo();
	create_foreign_on_ah_paseo(
			bridged_roc_at_asset_hub_paseo.clone(),
		true,
