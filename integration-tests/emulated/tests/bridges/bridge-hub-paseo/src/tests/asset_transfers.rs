...
fn set_up_dot_for_penpal_polkadot_through_pah_to_kah(
	...
	let dot_at_polkadot = dot_at_ah_polkadot();
...
fn send_dot_from_polkadot_relay_through_asset_hub_polkadot_to_asset_hub_kusama_to_penpal_kusama() {
	...
	let sender = PaseoSender::get();
	let receiver = PenpalAReceiver::get();
	let local_asset_hub = Paseo::child_location_of(AssetHubPolkadot::para_id());
...
fn send_back_ksm_from_penpal_paseo_through_asset_hub_paseo_to_asset_hub_kusama_to_penpal_kusama(
	...
	PenpalB::execute_with(|| {
		assert_ok!(<PenpalB as Chain>::System::set_storage(
			...
			vec![(
				PenpalCustomizableAssetFromSystemAssetHub::key().to_vec(),
				ksm_at_paseo_parachains.encode(),
			)],
		));
	});
	...
	let sender_ksm_before = PenpalB::execute_with(|| {
		type ForeignAssets = <PenpalB as PenpalBPallet>::ForeignAssets;
		<ForeignAssets as Inspect<_>>::balance(ksm_at_paseo_parachains.clone(), &sender)
	});
	...
	// send KSMs over the bridge, all fees paid with KSM along the way
	{
		...
		let xcm = Xcm::<()>(vec![
			WithdrawAsset((ksm_at_paseo_parachains.clone(), amount).into()),
			SetFeesMode { jit_withdraw: true },
			InitiateReserveWithdraw {
				...
				xcm: Xcm::<()>(vec![
					BuyExecution {
						fees: (ksm_at_paseo_parachains.clone(), amount / 2).into(),
						weight_limit: Unlimited,
					},
					InitiateReserveWithdraw {
						assets: Wild(AllCounted(1)),
				...
				assert_expected_events!(
					...
					vec![
						...
						RuntimeEvent::ForeignAssets(
							...
							asset_id: asset_id == &ksm_at_paseo_parachains,
	...
	let sender_ksm_after = PenpalB::execute_with(|| {
		type ForeignAssets = <PenpalB as PenpalBPallet>::ForeignAssets;
		<ForeignAssets as Inspect<_>>::balance(ksm_at_paseo_parachains, &sender)
	});
