...
//! Remaining Percentage Stepped Curve implementation.
//!
//!
//! This module provides a stepped curve system that allows values to move towards
//! a target asymptote over time-- Moving a percentage of the remaining distance
//! closer after each stepping period.
//!
//! For example, a curve from 0 to 100 at 50% stepping (not to scale):
//!
//! ```text
//!                     75   --------
//!                         |
...
//! ```
//!
//! Originally extracted as a more bespoke version of https://github.com/paritytech/polkadot-sdk/pull/9556
...
#[derive(PartialEq, Eq, Debug, TypeInfo, Clone, Default)]
...
#[cfg(test)]
mod stepped_curve_tests {
	use super::*;

	#[test]
	fn curve_returns_initial_value_before_start() {
		let curve = SteppedCurve::try_new(
