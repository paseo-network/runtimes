...
/// Identifiers of the external asset (pUSD, the coinage underlying asset) shared by both
/// runtimes.
///
/// `PROTECTED_ASSET_ID` is the pallet-assets u32 id of the asset on Asset Hub (where Assets is
/// keyed by u32). `ProtectedAssetLocation` is the XCM `Location` view of the same asset as seen
/// from the People chain (where Assets is Location-keyed); People re-exports it as
/// `ExternalAssetLocation` for coinage and Asset Hub re-anchors it locally in its XCM config.
pub mod protected_asset {
	use frame_support::parameter_types;
	use xcm::latest::{Junction, Location};

	/// External asset (pUSD) pallet-assets u32 id on Asset Hub.
	pub const PROTECTED_ASSET_ID: u32 = 50_000_413;

	parameter_types! {
		/// External asset (pUSD) pallet-assets u32 id, exposed as `Get<u32>`.
		pub const ProtectedAssetId: u32 = PROTECTED_ASSET_ID;
		/// External asset (pUSD) `Location`, as seen from a sibling of Asset Hub.
