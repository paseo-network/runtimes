...
fn main() -> Result<(), String> {
	...
	let supported_chains =
		HashMap::<_, Box<dyn Fn() -> Result<Box<dyn ChainSpec>, String>>>::from([
			("paseo", Box::new(|| relay_chain_specs::paseo_config()) as Box<_>),
			("paseo-local", Box::new(|| relay_chain_specs::paseo_local_config()) as Box<_>),
			(
				"asset-hub-paseo",
				Box::new(|| system_parachain_specs::asset_hub_paseo_testnet_config()) as Box<_>,
			),
			(
